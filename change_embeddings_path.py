...
for model in os.listdir(args.models_path):
    if args.filter != "" and args.filter not in model:
        continue
    with open(os.path.join(args.models_path, model, "config.json"), "r") as f:
        config = json.load(f)
    print(f"Changing embeddings path for {model} from {config['emb_folder']} "
          f"to {args.embeddings_path}")
    config["emb_folder"] = args.embeddings_path
    with open(os.path.join(args.models_path, model, "config.json"), "w") as f:
